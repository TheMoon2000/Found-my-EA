...
class DateScheduler: NSViewController, NSTableViewDelegate, NSTableViewDataSource {
    ...
    let EA_Name = mainViewController!.EA_Name.stringValue
    ...
    var dates = [String]() {
        didSet {
            ...
            for i in 0..<mainViewController!.myEAs.count {
                if mainViewController!.myEAs[i].name == EA_Name {
                    needsUpdate = false
                    ...
                    needsUpdate = true
                    mainViewController!.myEAs[i].dates = dates.joined(separator: " | ")
                    mainViewController!.changeStatusForKey("Dates", value: dates.joined(separator: " | "), spinner: NSProgressIndicator(), updateRow: false)
    ...
    override func viewDidLoad() {
        ...
        let theEA = mainViewController!.myEAs.filter({$0.name == EA_Name})[0]
        
        dates = theEA.dates == "" ? [String]() : theEA.dates.components(separatedBy: " | ")
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        formatter.locale = Locale(identifier: "en_US")
        formatter.dateFormat = "MMM. d, y"
        if let date = formatter.date(from: dates[row]) {
    ...
    @IBAction func deleteDate(_ sender: NSButton) {
        ...
        if !tmpDates.contains(mainViewController!.nextSessionDate) {
            mainViewController!.nextDate(sender)
        }
